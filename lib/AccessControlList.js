var debug = require('debug')('node-access-controls:AccessControlList');
...
AccessControlList.prototype.authorize = function(obj, action, roles, context, callback) {
  debug('roles in AccessControlList.prototype.authorize: %s', roles);
  var authorize = false
  var reason = ''
  var inherit = []
  ...
  if(shouldApply.ok) {
    ...
    debug('conditionsMatch.ok: %s', conditionsMatch.ok);
    ...
    if(conditionsMatch.ok === false) {
      ...
      debug('rolesMath: %s, this._roles: %s, roles: %s', rolesMatch, this._roles, roles);
  ...
  debug('authorize: %s, reason: %s', authorize, reason);
...
AccessControlList.prototype._rolesMatch = function(expectedRoles, actualRoles) {
  debug('expectedRoles: %s, actualRoles: %s', expectedRoles, actualRoles);
